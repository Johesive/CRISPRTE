...
class PGSQL:
    ...
    def select_gtf_by_tename(cursor, tename, tabname="te"):
        ...
        result = cursor.fetchall()
        result = list(map(lambda x: list(map(lambda z: z.tobytes().decode() if type(z) == memoryview else z, x)), result))
    ...
    @staticmethod
    def select_gtf_by_tename_(cursor, te_name, return_bed):
...
def query_mismatch_annotation(pcursor, **args):
    ...
    result = {}
    if len(targetTE_gids) > 100:
        targetTE_gids = targetTE_gids[:100]
